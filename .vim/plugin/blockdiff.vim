...
fun! BlockDiff_GetBlock1() range
  ...
  let s:block1 = @@
  let s:block1_name = expand('%') . '(' . a:firstline . '-' . a:lastline . ')' 
  echo s:block1_name
...
fun! BlockDiff_GetBlock2() range
  let s:regd = @@
  exe a:firstline . "," . a:lastline . "y"
  let s:block2_name = expand('%') . '(' . a:firstline . '-' . a:lastline . ')' 
  echo s:block2_name

  " Open new tab, paste second selected block
  exe "tabnew <1>" . s:block1_name
  " exe "tabnew " . 'block1'
  normal P
  ...
  exe "lefta vnew <2>" . s:block2_name
  " exe "lefta vnew " . 'block2'
